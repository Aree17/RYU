...
def resolver_prueba_view(request, prueba_usuario_id):
    prueba_usuario = get_object_or_404(
        PruebaUsuario,
    ...
    preguntas = (
        Pregunta.objects
        .filter(banco__periodoVigente=periodo)
    ...
    try:
        for key, value in request.POST.items():
            if key.startswith("pregunta_"):
                ...
                registrar_respuesta(
                    prueba_usuario_id=prueba_usuario_id,
...
def visualizar_resultados_view(request, prueba_usuario_id):
    ...
    prueba_usuario = get_object_or_404(
        PruebaUsuario,
        id=prueba_usuario_id,
        persona=cuenta.persona if cuenta else None
    )

    resultados = (
        ResultadoPorCarrera.objects
        .filter(prueba_usuario=prueba_usuario)
        .select_related("banco_pregunta__carrera")
